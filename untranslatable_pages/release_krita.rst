...
#. Template ticket for sysadmins for "Beta" releases (no store uploads):
    ...
    .. code::
        ...
        #) Upload release artifacts to download.kde.org:
            ...
            * Destination link: https://download.kde.org/unstable/krita/5.2.6/
        ...
        #) Add `Krita 5.2.6` bugzilla version

        PS:
        Here are the SHA-256 sums for the uploaded files:

        ```
        <here comes the data you generated by running `sha256sum *` in the release folder>
        ```

    .. warning::

        Please note that sysadmins ask for SHA-256 for the files to be included into the task
        itself. That allows them to verify consistency of the files they upload.
